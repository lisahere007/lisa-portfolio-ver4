...
export default function Hero() {
  ...
  return (
    <section style={{
      ...
      <div style={{ position: 'absolute', inset: 0, zIndex: 1, background: 'linear-gradient(to bottom, rgba(10,20,50,0.55) 0%, rgba(10,20,50,0.35) 50%, rgba(10,20,50,0.7) 100%)' }} />
      <div style={{ position: 'absolute', inset: 0, zIndex: 1, pointerEvents: 'none', backgroundImage: 'linear-gradient(rgba(100,150,220,0.06) 1px, transparent 1px), linear-gradient(90deg, rgba(100,150,220,0.06) 1px, transparent 1px)', backgroundSize: '56px 56px' }} />

      <div style={{
        position: 'relative', zIndex: 2, textAlign: 'center',
        padding: '0 20px', maxWidth: '900px', width: '100%',
      }}>
        <div style={{
          fontFamily: 'var(--mono)', fontSize: '11px', letterSpacing: '0.22em',
          color: 'rgba(255,255,255,0.55)', textTransform: 'uppercase', marginBottom: '28px',
        ...
        <div style={{
          fontFamily: 'var(--mono)', fontSize: 'clamp(13px, 2vw, 18px)',
          color: 'rgba(255,255,255,0.65)', marginBottom: '32px', height: '28px',
          animation: 'fadeUp 0.8s ease 0.6s both',
        }}>
          <span>{typed}</span>
          <span style={{ display: 'inline-block', width: '2px', height: '1em', background: 'rgba(255,255,255,0.6)', marginLeft: '2px', verticalAlign: 'middle', animation: 'blink 1s step-end infinite' }} />
        </div>

        <p style={{
          fontSize: 'clamp(13px, 1.5vw, 17px)', color: 'rgba(255,255,255,0.6)',
          lineHeight: 1.9, maxWidth: '600px', margin: '0 auto 48px',
        ...
        <div style={{
          ...
          <a href="#career" className="btn-primary">{lang === 'en' ? 'View Work' : '경력 보기'}</a>
          <a href="#contact" className="btn-ghost">{lang === 'en' ? 'Contact' : '연락하기'}</a>
        </div>
      ...
      <div style={{
        ...
        <span style={{ fontFamily: 'var(--mono)', fontSize: '10px', color: 'rgba(255,255,255,0.4)', letterSpacing: '0.12em' }}>SCROLL</span>
        <div style={{ width: '1px', height: '40px', background: 'linear-gradient(to bottom, rgba(255,255,255,0.4), transparent)', animation: 'scrollLine 2s ease-in-out infinite' }} />
      </div>
